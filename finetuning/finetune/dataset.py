    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        try:
            if isinstance(self.raw_data[i]["image"], str):
                ...
                image_path = self.raw_data[i]["image"]
                images_dict = { "<image>" : Image.open(image_path.replace("../../images/", "../../data/images/")).convert("RGB") }
            ...
            ret = preprocess(
                images_dict,
                self.raw_data[i]["lang"], # added 
                self.raw_data[i]["example_id"], # added 
...
def conversation_to_ids(lang, task_type, conversation, tokenizer, llm_type=None, new_schema=False, max_length=2048):
    """
    ...
    elif llm_type == "qwen2_mmiglu":
        input_ids, context, raw_msg = conversation_to_ids_qwen2_mmiglu(
            lang, task_type, conversation, tokenizer
        )
    else:
    ...
    # build target
    # inizialize all with -100 value
    target = torch.full_like(ids, -100, dtype=torch.int32)

    # Set the target to ids when the context is 0 and leave the rest set to -100
...
def conversation_to_ids_qwen2_mmiglu(lang, task_type, conversation, tokenizer):
    
    raw_msg = ""
    
    sys_prompt = get_system_prompt(task_type, lang)
    ...
    for idx, msg in enumerate(conversation):
        role = msg["role"]
        message = msg["content"]
        assert role in ["user", "assistant","plan"] 
        if role == "user":
            prefix = "user"
        elif role == "plan": #added role
            prefix = "plan" 
        else:
            prefix = "assistant"
        chat.append({"role":prefix, "content":message})
        raw_msg += prefix + message
    #assert set([i['role'] for i in chat]) & set(['assistant'])
    #assert set([i['role'] for i in chat]) & set(['plan']) # edited here
    ...
    start_idxs = np.where(input_ids == tokenizer.convert_tokens_to_ids('<|im_start|>'))[0]
    assistant_idxs = np.where(input_ids == tokenizer.convert_tokens_to_ids('assistant'))[0]
    plan_idxs = np.where(input_ids == tokenizer.convert_tokens_to_ids('plan'))[0] 
    end_idxs = np.where(input_ids == tokenizer.convert_tokens_to_ids('<|im_end|>'))[0]

    context = np.ones_like(input_ids, dtype=np.int8)
    
    if "plan" not in task_type:
        for assistant_idx in assistant_idxs:
            if assistant_idx-1 in set(start_idxs):
                st = assistant_idx + 1
                for end_idx in end_idxs:
        ...
        for plan_idx in plan_idxs: 
            if plan_idx-1 in set(start_idxs):
                st = plan_idx + 1
                for end_idx in end_idxs:
                    if end_idx > st:
                        context[st: end_idx + 1] = 0
...
def preprocess(
    ...
    new_schema = False
    use_image_id = False
    if llm_type=='qwen2' or llm_type=='qwen2_mmiglu': # edited
        new_schema = True
        use_image_id = True
    ...
    if len(images_dict) == 1 and "<image>" in images_dict:       
        ...
        input_dict = conversation_to_ids(lang, task_type, conversations, tokenizer, llm_type, new_schema, max_length)
    else:
        pattern = r'<image_\d+>'
        new_conversations = []

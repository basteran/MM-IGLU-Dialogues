...
def generate(messages_input, lang, lora_model, tokenizer):
...
def write_tsv_files(results, question_file, lang, lora_model, tokenizer):
    ...
    for instance in results:
        ...
        predicted_output = generate(messages_input, lang, lora_model, tokenizer)
        print("---------------------------------------------------------------")
...
def main():
    ...
    parser.add_argument("--path_to_adapter", default="../finetuning/finetune/output_history_noplan_en/output__lora_history_noplan_en", help="Path to the fine-tuned LoRA adapter")
    parser.add_argument("--name", default="TEST", help="Custom name for the output TSV file")

    args = parser.parse_args()

    input_file = f"../data/{'en' if args.language == 'en' or args.language == 'it2en' else 'it'}/history_noplan/dialogues_{'eng' if args.language == 'en' or args.language == 'it2en' else 'ita'}_test_question_history_noplan.json"
    question_tsv = f"{args.language.upper()}_{args.name}_generated_questions_noplan.tsv"
    ...
    write_tsv_files(formatted_msgs, question_tsv, args.language, lora_model, tokenizer)
